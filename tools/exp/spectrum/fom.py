...
from postprocessing.modulator import get_phase_shift_from_rshift, get_modeff_from_rshift
...
def main():
    ...
    for volt in voltages:
        ...
        phi.append((analysis.resonance_wavelength, analysis.fsr(num_peaks=2)))
    phi = np.array(phi)

    phase_shift = get_phase_shift_from_rshift(phi[:, 0], phi[:, 1])
    mod_eff = get_modeff_from_rshift(voltages, phi[:, 0], phi[:, 1], length)
