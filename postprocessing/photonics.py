...
Generic ring resonator / MZI physics formulas, shared by fitting and
simulation tooling.
"""
import numpy as np


def ring_intensity(a: float, t: float, phi: np.ndarray) -> np.ndarray:
    """
    All-pass ring resonator through-port intensity.

    Parameters:
        a (float): Round trip amplitude transmission.
        t (float): Self-coupling coefficient.
        phi (np.ndarray): Round trip phase.

    Returns:
        np.ndarray: The transmitted intensity.
    """
...
def ramzi_output_intensity(dphi: float, a: float, t: float, phi: np.ndarray) -> np.ndarray:
    """
    Ring-assisted MZI output intensity.
    ...
    Returns:
        np.ndarray: The output intensity.
    """
    ...
    transmission = ring_intensity(a, t, phi)

    return (transmission + 1 + 2*np.abs(field)*np.cos(dphi + ring_ph))/4


def mzi_output(I0: float, delta_phi: np.array) -> float:
    return I0/2*(1 + np.cos(delta_phi))


def mzi_delta_phi(neff: float, wavelength: float) -> float:
